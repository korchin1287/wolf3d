...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/08/25 19:02:23 by nofloren          #+#    #+#             */
/*   Updated: 2020/08/26 15:31:35 by nofloren         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_create_bmp_head(t_parser *pars, unsigned int size_bmp, int fd)
{
	unsigned int	i;
	...
	ft_convert(str + 18, pars->width);
	ft_convert(str + 22, pars->height);
...
void	create_bmp(t_data *data, t_parser *pars)
{
	...
	if ((fd = open("screenshot.bmp", O_CREAT | O_TRUNC | O_WRONLY | O_APPEND,
		0666)) == -1)
		ft_error(8);
	size_bmp = 54 + (pars->width * pars->height * 4);
	ft_create_bmp_head(pars, size_bmp, fd);
