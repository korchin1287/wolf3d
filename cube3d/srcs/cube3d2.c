...
/*   Updated: 2020/08/07 18:50:02 by nofloren         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	casting3(t_data *data)
{
	while (data->hit == 0)
	{
		...
		if (world_map[data->map_x][data->map_y] == 1)
			data->hit = 1;
	}
}
...
static void	casting4_2(t_data *data)
{
	if (world_map[data->map_x][data->map_y] == 2)
	{
		data->perp_wall_dist = sqrt((data->pos_x - data->map_x) * (data->pos_x - data->map_x) + (data->pos_y - data->map_y) * (data->pos_y - data->map_y));
		data->line_height = (int)(data->height / data->perp_wall_dist);
		data->draw_start = (int)(-(data->line_height) / 2 + data->height / 2);
		if (data->draw_start < 0)
			data->draw_start = 0;
...
static void	casting3_2(t_data *data)
{
	while (data->hit == 0)
	{
		if (data->side_dist_x < data->side_dist_y)
...
static void	ver_line(t_data *data, int color)
{
	int	i;
	int r;
...
static void		ft_side_wall(t_data *data)
{
	...
	//	ft_tex_n(data);
		ver_line(data, 0x00ff0000);
	if (data->map_x > data->map_x_start && data->side == 0 && (world_map[data->map_x][data->map_y] == 1))
		//ft_tex_s(data);
		ver_line(data, 0x0000ff00);
	if (data->map_y < data->map_y_start && data->side == 1 && (world_map[data->map_x][data->map_y] == 1))
		//ft_tex_e(data);
		ver_line(data, 0x000000ff);
	if (data->map_y > data->map_y_start && data->side == 1 && (world_map[data->map_x][data->map_y] == 1))
		//ft_tex_w(data);
		ver_line(data, 0x00ff00ff);
//	if (world_map[data->map_x][data->map_y] == 2)
//		ft_tex_sprite(data);
}
...
static void	ray_casting(t_data *data)
{
	...
	while (data->x < data->width)
	{
		...
		casting4_2(data);
		ft_tex_sprite(data);
		if (world_map[data->map_x][data->map_y] == 2 && data->count < data->line_height)
		{	
			verLine_sprite(data);
			data->count++;
		}
...
{
	//data->sprite->img = mlx_new_image(data->mlx, 640, 480);
